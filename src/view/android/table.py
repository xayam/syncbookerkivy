...
class Table(TabbedPanelItem):
    def __init__(self, app):
        ...
        TabbedPanelItem.__init__(self,
                                 background_normal=self.app.model.conf.ICON_TABLE,
                                 background_down=self.app.model.conf.ICON_TABLE_PRESSED)
        self.table_gridlayout = GridLayout(cols=3)
        self.table_navigator = GridLayout(rows=5,
                                          size_hint_x=0.3)
        self.table_prev = Button(background_normal=self.app.model.conf.ICON_PREV,
                                 background_down=self.app.model.conf.ICON_PREV_PRESSED,
                                 on_release=self.app.player.prev_button_click)
        self.table_navigator.add_widget(self.table_prev)
        self.table_play = Button(background_normal=self.app.model.conf.ICON_PLAY,
                                 background_down=self.app.model.conf.ICON_PLAY_PRESSED,
                                 on_release=self.app.player.play_button_click)
        self.table_navigator.add_widget(self.table_play)
        self.table_pause = Button(background_normal=self.app.model.conf.ICON_PAUSE,
                                  background_down=self.app.model.conf.ICON_PAUSE_PRESSED,
                                  on_release=self.app.player.pause_button_click)
        self.table_navigator.add_widget(self.table_pause)
        self.table_stop = Button(background_normal=self.app.model.conf.ICON_STOP,
                                 background_down=self.app.model.conf.ICON_STOP_PRESSED,
                                 on_release=self.app.player.stop_button_click)
        self.table_navigator.add_widget(self.table_stop)
        self.table_next = Button(background_normal=self.app.model.conf.ICON_NEXT,
                                 background_down=self.app.model.conf.ICON_NEXT_PRESSED,
        ...
        self.app.table_label_left = TextInput(size_hint=(1, None),
                                              focus=False,
                                              selection_color=self.app.model.opt[SEL],
                                              background_color=self.app.model.opt[BG],
                                              foreground_color=self.app.model.opt[FG],
                                              handle_image_left=self.app.model.conf.DISABLE_MARKER,
                                              handle_image_right=self.app.model.conf.DISABLE_MARKER,
    ...
    def update_table_label_left(self, _=None):
        self.app.model.log.debug("Enter to function 'update_table_label_left()'")
        ...
        if self.app.table_label_left.text == "\n" * 50:
            self.app.model.log.debug("True is self.app.table_label_left.text == '\n'*50")
            return
        self.app.model.log.debug("MySound().load_seek")
    ...
    def update_table_label_right(self, _=None):
        self.app.model.log.debug("Enter to function 'update_table_label_right()'")
        ...
        if self.app.table_label_left.text == "\n" * 50:
            self.app.model.log.debug("True is self.app.table_label_right.text == '\n'*50")
            return
        self.app.model.log.debug("MySound().load_seek")
        if self.app.model.opt[POSITIONS][self.app.model.current_select][AUDIO] == RU:
            try:
                self.app.model.sound.stop()
                self.app.model.clock_action.cancel()
            except AttributeError:
