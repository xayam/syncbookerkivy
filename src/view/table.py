...
class Table(TabbedPanelItem):
    ...
    def update_table_label_left(self, _=None):
        ...
        if self.app.opt[POSITIONS][self.app.current_select][AUDIO] == EN:
            ...
            self.app.log.debug(f"self.app.chunk_current={self.app.chunk_current}")
            self.app.log.debug("self.app.opt[POSITIONS][self.app.current_select][CHUNK]=" +
                               str(self.app.opt[POSITIONS][self.app.current_select][CHUNK]))
